#!/usr/bin/env node
// Writes the complete index.html for the Paw & Go SPA
// Run: node scripts/write-index-html.js
const fs = require('fs');
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .avail-row select, .avail-row input { padding: 6px 8px; border: 1px solid var(--border); border-radius: var(--radius); font-size: .85rem; }
    ...
    .schedule-card {
      ...
      padding: 14px 16px; display: grid; grid-template-columns: 68px 1fr; gap: 12px;
      align-items: start; margin-bottom: 8px;
    }
    .schedule-time {
      background: var(--primary); color: #fff; border-radius: var(--radius);
      padding: 6px 8px; font-weight: 700; font-size: .9rem; text-align: center;
    }
  ...
  try {
    ...
    Object.keys(byWalker).forEach(walker => {
      ...
      byWalker[walker].forEach(a => {
        ...
        const diffDropoff = a.dropoffStreet!==a.pickupStreet || a.dropoffCity!==a.pickupCity;
        if (diffDropoff && (a.dropoffStreet||a.dropoffCity)) {
...
function drawApptTable(filter) {
  ...
  const rows = apptCache.appointments.filter(a =>
    !lf || [fullName(a.walker),fullName(a.customer),a.date,a.timeSlot,a.status]
      .some(v => String(v||'').toLowerCase().includes(lf))
  );
  const s = {scheduled:0,confirmed:0,cancelled:0,completed:0};
  apptCache.appointments.forEach(a => { if (a.status in s) s[a.status]++; });
  let html =
    '<div class="stats-row">'+
    '  <div class="stat-card"><div class="num">'+apptCache.appointments.length+'</div><div class="lbl">Total</div></div>'+
    '  <div class="stat-card"><div class="num">'+s.scheduled+'</div><div class="lbl">Scheduled</div></div>'+
    '  <div class="stat-card"><div class="num">'+s.confirmed+'</div><div class="lbl">Confirmed</div></div>'+
  ...
  rows.forEach(a => {
    const dogNames = (a.dogs||[]).map(ad => ad.dog?ad.dog.name:'?').join(', ')||'—';
    let acts = '';
    if (a.status==='scheduled') acts += '<button class="btn btn-sm btn-primary" onclick="patchAppt(\\''+a.ID+'\\',\\'confirmed\\')">Confirm</button> ';
    if (a.status==='confirmed') acts += '<button class="btn btn-sm btn-success" onclick="patchAppt(\\''+a.ID+'\\',\\'completed\\')">&#x2714; Complete</button> ';
    if (a.status!=='cancelled'&&a.status!=='completed') acts += '<button class="btn btn-sm btn-danger" onclick="cancelApptConfirm(\\''+a.ID+'\\')">Cancel</button>';
    html += '<tr>'+
      '<td>'+esc(a.date)+'</td>'+
...
function openApptModal() {
  const today = new Date().toISOString().slice(0,10);
  const slotOpts = apptCache.slots.map(s => { const sv=typeof s==='string'?s:(s.slot||''); return '<option value="'+esc(sv)+'">'+esc(sv)+'</option>'; }).join('');
  ...
  document.body.insertAdjacentHTML('beforeend',
    ...
    '  <div class="form-group"><label>Dogs (Ctrl/Cmd = multi-select)</label><select id="af-dogs" multiple size="5" style="height:110px"><option disabled>Select a customer first</option></select></div>'+
...
async function saveAppt() {
  ...
  try {
    await apiFetch('/Appointments',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({date,timeSlot:slot,walker_ID:wid,customer_ID:cid,notes:notes||null,
        dogs:dogIds.map(id=>({dog_ID:id}))})});
    closeModal('appt-modal'); toast('Appointment booked — confirmation auto-created!'); renderAppointments();
...
async function patchAppt(id,status) {
  try {
    ...
    toast('Appointment '+status+'.'); renderAppointments();
  } catch(e) { toast(e.message,'error'); }
}
  ...
  try {
    ...
    walkers.forEach(w => {
      ...
      rows+='<tr>'+
        ...
        '<td><button class="btn btn-sm btn-ghost" onclick="openWalkerModal(\\''+enc+'\\')">Edit</button></td>'+
        '</tr>';
    });
    main.innerHTML=
      '<div class="card">'+
...
function renderAvailList() {
  ...
  if (!_walkerAvailEdits.length) { c.innerHTML='<div style="color:var(--text-muted);font-size:.85rem">No slots.</div>'; return; }
...
async function renderCustomers() {
  ...
  try {
    ...
    custs.forEach(c=>{
      ...
      const addrCnt=(c.addresses||[]).length;
      ...
      rows+='<tr>'+
        ...
        '<td>'+dogBadges+'</td>'+
        '<td>'+badge(addrCnt,'blue')+'</td>'+
        '<td><button class="btn btn-sm btn-ghost" onclick="openCustModal(\\''+enc+'\\')">Edit</button></td>'+
        '</tr>';
    });
    main.innerHTML=
      '<div class="card">'+
      '  <div class="card-header"><h2>&#x1F465; Customers ('+custs.length+')</h2>'+
      ...
      '  <tbody>'+(rows||'<tr><td colspan="7" class="empty">No customers.</td></tr>')+'</tbody>'+
      '  </table></div></div>';
  } catch(e) { document.getElementById('main').innerHTML='<div class="card"><div class="empty" style="color:var(--danger)">'+esc(e.message)+'</div></div>'; }
}
var _custAddrEdits=[];
  ...
  try {
    ...
    _allDogs.forEach(d=>{
      ...
      rows+='<tr>'+
        ...
        '<td>'+esc(fullName(d.owner)||'—')+'</td>'+
        '<td><small>'+esc(d.notes||'—')+'</small></td>'+
        '<td>'+
        '  <button class="btn btn-sm btn-ghost" onclick="openDogModal(\\''+enc+'\\')">Edit</button>'+
        '  <button class="btn btn-sm btn-ghost" style="margin-left:4px" onclick="openFriendsModal(\\''+enc+'\\')">&#x1F43E; Friends</button>'+
        '</td>'+
        '</tr>';
    });
    main.innerHTML=
      '<div class="card">'+
      '  <div class="card-header"><h2>&#x1F436; Dogs ('+_allDogs.length+')</h2>'+
...
async function saveDog(id) {
  ...
  const body={name,breed:document.getElementById('df-breed').value.trim()||null,
...
async function openFriendsModal(encoded) {
  ...
  const friendIds=new Set(friends.map(f=>f.friend_ID||f.friend&&f.friend.ID));
  ...
  const chips=friends.map(f=>{
    ...
    return '<span class="friend-chip">'+esc(fn)+
      '<button onclick="removeFriend(\\''+f.ID+'\\')">&#x2715;</button></span>';
  }).join('');
  document.body.insertAdjacentHTML('beforeend',
    '<div class="modal-backdrop" id="friends-modal" onclick="closeModal(\\'friends-modal\\')">'+
    '<div class="modal" onclick="event.stopPropagation()">'+
    '  <h3>&#x1F43E; Dog Friends — '+esc(dog.name)+'</h3>'+
    '  <div class="sub-section" style="margin-top:0;border-top:none;padding-top:0">'+
    '    <h4>Current Friends</h4>'+
    '    <div id="friends-chips">'+(chips||'<span style="color:var(--text-muted);font-size:.85rem">No friends yet.</span>')+'</div>'+
    '  </div>'+
    '  <div class="form-group" style="margin-top:14px"><label>Add Friend</label>'+
    '    <select id="ff-dog"><option value="">— Select dog —</option>'+dogOpts+'</select>'+
    '  </div>'+
    '  <div class="form-actions">'+
    '    <button class="btn btn-ghost" onclick="closeModal(\\'friends-modal\\')">Close</button>'+
    '    <button class="btn btn-primary" onclick="addFriend(\\''+esc(dog.ID)+'\\')">Add Friend</button>'+
    '  </div>'+
    '</div></div>');
}
async function addFriend(dogId) {
  const friendId=document.getElementById('ff-dog').value;
  if (!friendId) return toast('Select a dog','error');
  try {
    await apiFetch('/DogFriends',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({dog_ID:dogId,friend_ID:friendId})});
    toast('Friend added!');
    closeModal('friends-modal');
    const enc=encodeURIComponent(JSON.stringify({ID:dogId,name:''}));
    // Reopen with fresh data — find full dog object
    const freshDog=_allDogs.find(d=>d.ID===dogId)||{ID:dogId,name:'?'};
...
async function removeFriend(friendRowId) {
  ...
  try {
    await apiFetch('/DogFriends/'+friendRowId,{method:'DELETE'});
    toast('Friend removed.');
    // Refresh chips — close & let user reopen
    closeModal('friends-modal');
    renderDogs();
  } catch(e) { toast(e.message,'error'); }
}
...
async function renderBilling() {
  ...
  try {
    ...
    recs.forEach(r=>{ totalAmt+=(r.amount||0); if(r.status==='paid') paidAmt+=(r.amount||0); });
    let rows='';
    recs.forEach(r=>{
      const cn=fullName(r.appointment&&r.appointment.customer);
      const wn=fullName(r.appointment&&r.appointment.walker);
      const markBtn=r.status!=='paid'?'<button class="btn btn-sm btn-primary" onclick="markPaid(\\''+r.ID+'\\')">Mark Paid</button>':'';
      rows+='<tr>'+
    ...
    main.innerHTML=
      ...
      '  <div class="table-wrap"><table>'+
      '  <thead><tr><th>Issued</th><th>Customer</th><th>Walker</th